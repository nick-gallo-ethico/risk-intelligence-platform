...
**Note on forwardRef() for circular dependency:**
ChatbotModule imports AiModule (for ConversationService in EscalationService), and AiModule imports ChatbotModule (for FaqService in SkillRegistry). The forwardRef() resolves this at runtime. See verification step below.
...
5. Register escalate skill in skill.registry.ts (add with other chatbot skills)
   ...
   **Verify no circular dependency errors at runtime:**
   ```bash
   # Start backend in dev mode and check for circular dependency warnings
   cd apps/backend && npm run start:dev 2>&1 | head -50

   # Should NOT see errors like:
   #   "Nest can't resolve dependencies of..."
   #   "A circular dependency has been detected..."
   # If the app starts successfully and logs "Nest application successfully started", circular dependency is resolved.
   ```
   </verify>
   <done>EscalateSkill registered and available for chatbot agent. Circular dependency resolved via forwardRef() and verified at runtime.</done>
   </task>
