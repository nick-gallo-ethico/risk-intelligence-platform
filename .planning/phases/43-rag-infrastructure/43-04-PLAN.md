...
must_haves:
  truths:
    - "VectorStoreService uses raw SQL with explicit organizationId filter"
    - "Similarity search returns ranked chunks with distance scores"
    - "Upsert operation replaces existing chunks for a source"
    - "pgvector-node handles vector serialization"
  artifacts:
