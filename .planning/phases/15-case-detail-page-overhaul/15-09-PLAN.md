...
Runtime verification (manual): With backend running, test endpoint returns 200 with structured data:
  `curl -H "Authorization: Bearer $TOKEN" http://localhost:3000/api/v1/rius/{test-riu-id}/form-data`
  Expected: 200 response with { riuId, riuType, sections: [...] }
  </verify>
