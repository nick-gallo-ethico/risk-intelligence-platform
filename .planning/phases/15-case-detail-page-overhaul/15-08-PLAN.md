...
API endpoint: POST /investigation-interviews (existing backend)

- Requires investigationId - for MVP, if case has investigations, use first one
- If no investigations on case, show warning message and allow logging as a case activity instead
...
API approach for MVP:

- Log the attachment intent as an activity (similar to email logging)
- Full file upload integration with Azure Blob is complex - for now, log metadata only
...
Post to: POST /cases/:id/activity with action: 'document_attached'
...
POST /cases/:id/activity with action: 'task_created' and metadata containing task details.
