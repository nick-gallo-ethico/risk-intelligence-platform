...
apps/frontend/src/components/policies/policy-version-diff.tsx
  ...
  <action>
**PREREQUISITE: Install diff library before implementing:**

```bash
cd apps/frontend && npm install diff @types/diff
```
...
```
  ...
  <verify>Components compile. `npm install diff @types/diff` completed successfully. Diff shows additions green, deletions red. Toggle switches modes.</verify>
