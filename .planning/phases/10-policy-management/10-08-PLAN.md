...
apps/frontend/src/components/policies/policy-editor.tsx
  ...
  <action>
**PREREQUISITE CHECK:** Before implementing, verify RichTextEditor exists:

```bash
# Check if RichTextEditor component exists
ls apps/frontend/src/components/rich-text/rich-text-editor.tsx 2>/dev/null || ls apps/frontend/src/components/ui/rich-text-editor.tsx 2>/dev/null || ls apps/frontend/src/components/editor/*.tsx 2>/dev/null
```

**If RichTextEditor does NOT exist**, create a minimal one first at `apps/frontend/src/components/rich-text/rich-text-editor.tsx`:
...
Ensure Tiptap dependencies are installed: `npm install @tiptap/react @tiptap/starter-kit @tiptap/extension-link @tiptap/extension-placeholder`
...
- Show loading state while fetching
  ...
  <verify>Editor compiles. Autosave triggers after typing stops. Rich text renders. Status banners show correctly. RichTextEditor component exists and is properly imported.</verify>
