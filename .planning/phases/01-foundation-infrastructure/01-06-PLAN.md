...
<task type="auto">
  ...
  <action>
    ...
    5. Update indexing processor to actually index (wire the placeholder from Plan 02):
       In `apps/backend/src/modules/jobs/processors/indexing.processor.ts`:
       - Add imports: `import { IndexingService } from '../../search/indexing/indexing.service';`
       - Add imports: `import { PrismaService } from '../../prisma/prisma.service';`
       - Inject services in constructor: `constructor(private indexingService: IndexingService, private prisma: PrismaService)`
       - In `process()` method for 'create'/'update' operations:
         ```typescript
         // Load entity from database
         const entity = await this.loadEntity(job.data.entityType, job.data.entityId);
         if (!entity) return;
         // Transform to search document
         const document = this.transformToDocument(job.data.entityType, entity);
         // Index the document
         ...
         ```
       - Add helper method `loadEntity(entityType: string, entityId: string)` that uses Prisma to load Case, RIU, etc.
       - Add helper method `transformToDocument(entityType: string, entity: any)` that maps entity fields to index mapping
  </action>
  <verify>
