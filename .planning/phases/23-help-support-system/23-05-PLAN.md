...
<verify>Run `cd apps/backend && npx tsc --noEmit` to verify seed script compiles. Run `cd apps/frontend && npx tsc --noEmit` to verify contextual help component compiles. Check that the seed script is registered in index.ts.</verify>
<done>15+ knowledge base articles seeded across 7 categories with realistic HTML content. Contextual help link component created. Seed script is idempotent and registered in the seed index.</done>
</task>
