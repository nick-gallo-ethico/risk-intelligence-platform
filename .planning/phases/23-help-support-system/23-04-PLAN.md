...
must_haves:
  truths:
    - "Users can navigate to /help/tickets/new and submit a support ticket with subject, description, priority, and optional file attachments (screenshots)"
...
5. **Attachments** — File upload area for screenshots/attachments:
   - Use a simple `<input type="file" multiple accept="image/*,.pdf,.doc,.docx,.txt" />` wrapped in a styled drop zone (dashed border area with "Drag files here or click to upload" text and a Paperclip icon from lucide-react).
   - Store selected files in component state via `useState<File[]>([])`.
   - Show selected file names with a remove button (X icon) for each.
   - Limit: max 5 files, max 10MB each. Validate on selection and show error toast if exceeded.
   - Files are uploaded AFTER ticket creation succeeds (not part of the form mutation itself). After `createTicket` returns the new ticket with its `id`, upload each file by calling the existing storage controller's upload endpoint: `POST /api/v1/storage/upload` with `entityType=SUPPORT_TICKET` and `entityId=ticketId`. Reference the upload pattern used in the existing storage controller at `apps/backend/src/modules/storage/storage.controller.ts`. Use `FormData` with the file appended.
   - If any file upload fails, show a warning toast but still redirect (ticket is already created).

Submit button: "Submit Ticket" — disabled while submitting, shows loading spinner.

Use `useMutation` from @tanstack/react-query to call `helpApi.createTicket(data)`. On success, upload any attached files sequentially, then redirect.

On success:

- Upload any attached files to storage endpoint with `entityType: 'SUPPORT_TICKET'` and `entityId: createdTicket.id`
...
- Users can submit tickets with validated form (subject min 5 chars, description min 20 chars) and optional file attachments
- File attachments are uploaded to storage with entityType=SUPPORT_TICKET after ticket creation
