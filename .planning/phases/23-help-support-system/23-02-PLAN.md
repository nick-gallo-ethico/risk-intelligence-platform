...
**navigation.ts** — If this file contains a navigation items array or NavItem configuration, add a Help & Support entry pointing to `/help` with `HelpCircle` icon. Check the existing structure and match it. If navigation.ts doesn't control sidebar items (sidebar is hardcoded), skip this file.
</action>
