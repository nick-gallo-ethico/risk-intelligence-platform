...
<task type="auto">
  <name>Task 1: ReportController endpoints 1-6 (field discovery + basic CRUD)</name>
  <files>
    apps/backend/src/modules/analytics/reports/report.controller.ts
  </files>
  <action>
...
Inject: ReportService, ReportFieldRegistryService.
...
Note: This task creates the controller file. Endpoints 7-12 will be added by Task 2 in the same file.
</action>
<verify>
`cd apps/backend && npx tsc --noEmit --pretty` compiles without type errors.
Controller file exists with 6 endpoint methods.
</verify>
<done>ReportController provides field discovery and basic CRUD endpoints (GET fields, GET/POST/PUT reports, GET templates).</done>
</task>

<task type="auto">
  <name>Task 2: ReportController endpoints 7-12 (execution + actions + AI generation)</name>
  <files>
    apps/backend/src/modules/analytics/reports/report.controller.ts
  </files>
  <action>
Before implementing endpoint 12 (AI generation), verify AiQueryService is importable from the analytics AI query module. Check the existing ai-query module structure: look at `apps/backend/src/modules/analytics/ai-query/ai-query.module.ts` to confirm AiQueryService is exported. If AiQueryModule does not export AiQueryService, add it to the module's exports array. Then add AiQueryModule to the ReportModule imports array (this will be finalized in Task 3, but the import statement in the controller file can reference AiQueryService now).

Inject AiQueryService into the ReportController constructor alongside the existing ReportService and ReportFieldRegistryService injections.

Add these 6 endpoints to the existing ReportController:
...
Add Swagger decorators (@ApiTags, @ApiOperation, @ApiResponse, @ApiBearerAuth, @ApiParam) to all endpoints.
</action>
<verify>
`cd apps/backend && npx tsc --noEmit --pretty` compiles without type errors.
Controller exports all 12 endpoint methods.
AiQueryService is successfully injected (no missing provider errors).
</verify>
<done>ReportController provides complete REST API for report operations including delete, execution, duplicate, favorite, export, and AI generation. AiQueryService is verified importable and injected.</done>
</task>

<task type="auto">
  <name>Task 3: ReportModule and AnalyticsModule integration</name>
  <files>
    apps/backend/src/modules/analytics/reports/report.module.ts
...
@Module({
  imports: [
    ...
    // AiQueryModule for the ai-generate endpoint
    AiQueryModule,
  ],
...
The AiQueryModule import is required because ReportController injects AiQueryService (used for the ai-generate endpoint). Verify AiQueryModule exports AiQueryService -- if it does not, update AiQueryModule's exports array to include AiQueryService before importing it in ReportModule.
...
<done>ReportModule is wired into AnalyticsModule. All services are properly provided and exported. AiQueryModule is imported so AiQueryService is injectable in ReportController. The /api/v1/reports/\* routes are registered.</done>
</task>
...
- AiQueryModule is imported by ReportModule, making AiQueryService injectable in ReportController
