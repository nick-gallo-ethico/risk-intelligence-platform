...
<verify>
Run `cd apps/frontend && npx tsc --noEmit 2>&1 | head -30` to verify TypeScript compilation. Verify the new page file exists with `ls apps/frontend/src/app/(authenticated)/campaigns/new/page.tsx`. Verify navigation entries contain "Campaigns" and "Forms" by running `grep -n "title.*Campaigns\|title.*Forms" apps/frontend/src/lib/navigation.ts` and confirming both entries appear in the output.
</verify>
