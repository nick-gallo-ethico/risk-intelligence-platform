...
export class NotificationService {
  ...
  async queueEmail(params: QueueEmailParams): Promise<void> {
    ...
    const prefs = await this.routerService.getEffectivePreferences(
      recipientUserId,
      organizationId,
      category,
    );
    if (!this.routerService.shouldSendEmail(prefs, isUrgent)) {
      this.logger.debug(
        `Skipping email for user ${recipientUserId}: preference or quiet hours`,
      );
      return;
    }

    const actualRecipientId = this.routerService.resolveActualRecipient(
      recipientUserId,
      prefs,
      isUrgent,
    );
  ...
  async sendInApp(params: SendInAppParams): Promise<InAppNotification | null> {
    ...
    return this.dispatcherService.dispatchInApp({
      organizationId,
      recipientUserId,
      category,
      title,
  ...
  async notify(params: NotifyParams): Promise<void> {
    ...
    const routing = await this.routerService.determineRouting({
      organizationId,
      recipientUserId,
      category,
      isUrgent,
  ...
  async markAsRead(
    ...
    await this.prisma.notification.updateMany({
      where: { organizationId, userId, id: { in: notificationIds } },
      data: { isRead: true, readAt: new Date() },
    });
    const unreadCount = await this.dispatcherService.getUnreadCount(
      organizationId,
      userId,
    );
    this.dispatcherService.emitUnreadCountUpdate(
      organizationId,
      userId,
      unreadCount,
    );
