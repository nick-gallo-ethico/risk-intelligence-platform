...
export function PolicyTranslationsPanel({
  ...
  const createMutation = useMutation({
    mutationFn: ({ versionId, languageCode }: { versionId: string; languageCode: string }) =>
      policiesApi.createTranslation(versionId, languageCode, true),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['policy-translations', latestVersionId] });
  ...
  const refreshMutation = useMutation({
    mutationFn: (translationId: string) => policiesApi.refreshTranslation(translationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['policy-translations', latestVersionId] });
    },
  });
  ...
  const existingLanguages = new Set(translations?.map((t) => t.languageCode) || []);
  ...
  const handleAddTranslation = () => {
    if (latestVersionId && selectedLanguage) {
      createMutation.mutate({ versionId: latestVersionId, languageCode: selectedLanguage });
    }
  };
          ...
          <DialogContent>
            ...
            <div className="space-y-4 mt-4">
              ...
              <p className="text-sm text-muted-foreground">
                AI will generate an initial translation that you can review and edit.
              </p>
...
function TranslationCard({ translation, onRefresh, isRefreshing }: TranslationCardProps) {
  const languageName = SUPPORTED_LANGUAGES[translation.languageCode] || translation.languageName;
    ...
    <Card className={cn(translation.isStale && 'border-yellow-400')}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-2">
          ...
          <Badge className={cn('border', STATUS_COLORS[translation.reviewStatus])}>
            {STATUS_LABELS[translation.reviewStatus] || translation.reviewStatus}
          </Badge>
        </div>
