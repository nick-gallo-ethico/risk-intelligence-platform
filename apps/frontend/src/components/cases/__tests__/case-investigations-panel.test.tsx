...
import { CaseInvestigationsPanel, CaseInvestigationsPanelSkeleton } from '../case-investigations-panel';
import * as investigationApi from '@/lib/investigation-api';
import type { Case } from '@/types/case';
import type { Investigation, InvestigationListResponse } from '@/types/investigation';
...
describe('CaseInvestigationsPanel', () => {
  beforeEach(() => {
    ...
    (investigationApi.getInvestigationsForCase as ReturnType<typeof vi.fn>).mockResolvedValue(
      mockResponse
    );
  });
  ...
  it('renders empty state when no investigations', async () => {
    ...
    expect(screen.getByText('Create an investigation to begin')).toBeInTheDocument();
  });
  ...
  it('shows Create Investigation button in header', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('create-investigation-button')).toBeInTheDocument();
    });
  });
  ...
  it('fetches investigations from API on mount', async () => {
    ...
    await waitFor(() => {
      expect(investigationApi.getInvestigationsForCase).toHaveBeenCalledWith('case-456');
    });
  });

  it('shows error state when API fails', async () => {
    (investigationApi.getInvestigationsForCase as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  it('retries fetching investigations when Retry is clicked', async () => {
    ...
    await waitFor(() => {
      expect(investigationApi.getInvestigationsForCase).toHaveBeenCalledTimes(2);
    });
  ...
  it('shows AI summary when available', async () => {
    ...
    render(<CaseInvestigationsPanel caseData={caseWithSummary} isLoading={false} />);
