...
import { CaseActivityTimeline, CaseActivityTimelineSkeleton } from '../case-activity-timeline';
...
describe('CaseActivityTimeline', () => {
  ...
  it('renders Add Note button that opens modal when clicked', async () => {
    ...
    expect(screen.getByText('Note creation will be implemented in a future task.')).toBeInTheDocument();
  });
  ...
  it('renders activity entries after loading', async () => {
    ...
    expect(screen.getByText('John Doe changed status from NEW to OPEN')).toBeInTheDocument();
    expect(screen.getByText('Sarah Smith added a note')).toBeInTheDocument();
    expect(screen.getByText('Mike Johnson uploaded evidence.pdf')).toBeInTheDocument();
  });
  ...
  it('filters activities when Notes tab is clicked', async () => {
    ...
    expect(screen.getByText('Sarah Smith added a note')).toBeInTheDocument();
    expect(screen.queryByText('Jane Admin created the case')).not.toBeInTheDocument();
    expect(screen.queryByText('John Doe changed status from NEW to OPEN')).not.toBeInTheDocument();
  });
  ...
  it('filters activities when Status Changes tab is clicked', async () => {
    ...
    expect(screen.getByText('John Doe changed status from NEW to OPEN')).toBeInTheDocument();
    expect(screen.queryByText('Sarah Smith added a note')).not.toBeInTheDocument();
  });
  ...
  it('shows empty state when no activities match filter', async () => {
    const activitiesWithoutNotes = mockActivities.filter(a => a.action !== 'commented');
    const mockResponse: ActivityListResponse = {
  ...
  it('closes modal when Close button is clicked', async () => {
    ...
    expect(screen.queryByText('Note creation will be implemented in a future task.')).not.toBeInTheDocument();
  });
